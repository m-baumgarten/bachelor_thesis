    ...
    if QMin['template']['pyscf'] or QMin['template']['pure-pyscf']:   
        ...
        if 'h' in QMin:
            ...
            for istate,i in enumerate(QMin['statemap'].values()):   #optimize this loop
                ...
                mol_obj = get_energy(QMin,mult,state-1,gssorb = get_gssfile(QMin,job,mult=mult))
                if ss:
            ...
            for istate,i in enumerate(QMin['statemap'].values()):
                for jstate,j in enumerate(QMin['statemap'].values()):
                    ...
                    if mult1 == mult2 and state1 == state2 and ms1 == ms2:
                        ...
                        mol_obj= get_energy(QMin,mult1,state1-1,gssorb = get_gssfile(QMin,job,mult=mult1))
                        if ss:
...
def stripWORKDIR(WORKDIR):
    ...
    keep=['MOLCAS.out','MOLCAS\.[1-9]\.JobIph','MOLCAS\.[1-9]\.RasOrb','MOLCAS\.[1-9]\.molden','MOLCAS.rasscf.h5','MOLCAS.gssorb','MOLCAS\.[1-9]\.h5']
    for ifile in ls:
        delete=True
        for k in keep:
...
def get_gssfile(QMin,job,mult=None):
    if not QMin['template']['no-molcas-init']:
        if QMin['method']==0:
            ...
            #gssfile = os.path.join(QMin['scratchdir'],'master','MOLCAS.rasscf.h5')
            if job=='master':
                gssfile = os.path.join(QMin['scratchdir'],job,'MOLCAS.%i.h5' %mult)
            else:
                gssfile = os.path.join(QMin['scratchdir'],job,'MOLCAS.rasscf.h5')
        else:
